...
function Home() {
    return (<main className="min-h-screen bg-gradient-to-b from-gray-900 to-gray-800 text-white">
      {/* Header */}
      <header className="border-b border-gray-700 bg-gray-900/80 backdrop-blur-sm">
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <nav className="flex gap-4 text-sm">
            <link_1.default href="/status" className="hover:text-blue-400 transition">Status</link_1.default>
      ...
      <section className="max-w-7xl mx-auto px-4 py-16 text-center">
        ...
        {/* Test Button */}
        <div className="mb-12">
          <a href="/api/test-click?campaign=test-campaign" className="inline-block bg-green-600 hover:bg-green-500 text-white font-bold py-3 px-6 rounded-lg transition">
            🧪 Test Click Pipeline
          </a>
      ...
      <section className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="grid md:grid-cols-3 gap-4">
          <ComponentCard title="Pipeline Stages" count="22" items={['FindCampaignStage', 'ChooseStreamStage', 'ChooseOfferStage', 'ExecuteActionStage', 'StoreRawClicksStage', 'BuildRawClickStage']}/>
          <ComponentCard title="Actions" count="18" items={['HttpRedirect', 'Meta', 'DoubleMeta', 'Iframe', 'Frame', 'Curl', 'Remote', 'LocalFile', 'ShowHtml']}/>
          <ComponentCard title="Repositories" count="10" items={['CampaignRepository', 'OfferRepository', 'StreamRepository', 'ClickRepository', 'CachedCampaignRepository']}/>
        </div>
      </section>
...
function StatCard({ label, value }) {
    return (<div className="bg-gray-800/50 rounded-xl p-4 border border-gray-700">
      <div className="text-3xl font-bold text-blue-400">{value}</div>
      <div className="text-sm text-gray-400">{label}</div>
    </div>);
}
function ComponentCard({ title, count, items }) {
    return (<div className="bg-gray-800/30 rounded-xl p-4 border border-gray-700">
