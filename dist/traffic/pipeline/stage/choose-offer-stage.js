...
exports.ChooseOfferStage = void 0;
...
class ChooseOfferStage {
    static VERSION_SEND_TOKEN_ONLY = 2;
    static IGNORE_OFFER_PARAM = 'exit';
    /**
    ...
    process(payload, logEntry) {
        ...
        if (!response) {
            throw new stage_interface_js_1.StageException('response is not set', 'ChooseOfferStage');
        }
        if (!serverRequest) {
            throw new stage_interface_js_1.StageException('serverRequest is empty', 'ChooseOfferStage');
        }
        // No stream - skip
        if (!stream) {
        ...
        if (!campaign) {
            throw new stage_interface_js_1.StageException('Empty campaign', 'ChooseOfferStage');
        }
        if (!rawClick) {
            throw new stage_interface_js_1.StageException('Empty rawClick', 'ChooseOfferStage');
        }
        ...
        if (schema !== base_stream_js_1.StreamSchema.LANDING_OFFER && schema !== base_stream_js_1.StreamSchema.OFFER) {
    ...
    _getRandom(_request, associations, _logger) {
        ...
        if (associations.length === 1)
            return associations[0]?.offer ?? null;
     ...
     */
    _streamHasOfferId(_stream, _offerId) {
     ...
     */
    _findAvailableOffer(offer) {
        // TODO: Implement conversion capacity service
        return offer;
    }
}
exports.ChooseOfferStage = ChooseOfferStage;
//# sourceMappingURL=choose-offer-stage.js.map