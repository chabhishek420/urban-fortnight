...
export class UrlService extends AbstractService {
   ...
   */
  public getBaseUrl(uri: URL, strip: number = 2): string {
   ...
   */
  public getBasePath(uri: URL, depth: number = 1): string {
   ...
   */
  public stripHostWww(uri: URL): string {
   ...
   */
  public getBasePathWithSlash(uri: URL, depth: number = 1): string {
   ...
   */
  public addParameterToUrl(oldUrl: string, addToQuery: string): string {
    try {
   ...
   */
  public filterDoubleSlashes(url: string): string {
   ...
   */
  public parseStr(str: string): Record<string, string> {
   ...
   */
  public buildQuery(params: Record<string, string | number | boolean>): string {
    const searchParams = new URLSearchParams();
   ...
   */
  public joinParts(...parts: string[]): string {
    let result = parts[0] ?? '';
