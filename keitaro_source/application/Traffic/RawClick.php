...
class RawClick implements RawClickInterface
{
    ...
    public function setIpString($ip)
    {
        ...
        $this->setIp(Tools\Tools::ip2long($ip));
    }
    ...
    {
        if (!empty($test)) {
            throw new \Core\Application\Exception\Error("Use setSubIdN");
        }
    ...
    {
        if (!empty($currency) && $currency != Repository\CachedSettingsRepository::instance()->get("currency")) {
    ...
    {
        if (!$this->_stream) {
            $this->_stream = \Component\Streams\Repository\StreamRepository::instance()->find($this->get("stream_id"));
        }
        return $this->_stream;
    }
    ...
    {
        return in_array($this->getDeviceType(), [Device\DeviceType::DEVICE_TABLET, Device\DeviceType::DEVICE_MOBILE]);
    }
    ...
    {
        if (!empty($num)) {
            Logging\Service\LoggerService::instance()->error("Please use Visitor->getSubIdN() instead of Visitor->getSubId()");
    ...
    public function serialize()
    {
        ...
        $data = ["visitor_code" => $this->getVisitorCode(), "campaign_id" => $this->getCampaignId(), "stream_id" => $this->getStreamId(), "destination" => $this->getFilteredDestination(), "landing_id" => $this->getLandingId(), "landing_url" => $this->getLandingUrl(), "offer_id" => $this->getOfferId(), "affiliate_network_id" => $this->getAffiliateNetworkId(), "ip" => $this->getIp(), "ip_string" => $this->getIpString(), "datetime" => $this->getDateTime()->format(\Core\Model\AbstractModel::DATETIME_FORMAT), "user_agent" => $this->getUserAgent(), "language" => substr($this->getLanguage(), 0, 2), "source" => $this->getSource(), "x_requested_with" => $this->getXRequestedWith(), "keyword" => $this->getKeyword(), "referrer" => mb_substr($this->getReferrer(), 0, REFERRER_LIMIT), "search_engine" => $this->getSearchEngine(), "is_mobile" => $this->isMobile() ? 1 : 0, "is_bot" => $this->isBot() ? 1 : 0, "is_using_proxy" => $this->isUsingProxy() ? 1 : 0, "is_empty_referrer" => empty($referrer), "is_unique_campaign" => $this->isUniqueCampaign() ? 1 : 0, "is_unique_stream" => $this->isUniqueStream() ? 1 : 0, "is_unique_global" => $this->isUniqueGlobal() ? 1 : 0, "is_geo_resolved" => $this->isGeoResolved() ? 1 : 0, "is_device_resolved" => $this->isDeviceResolved() ? 1 : 0, "is_isp_resolved" => $this->isIspResolved() ? 1 : 0, "cost" => $this->getCost(), "sub_id" => $this->getSubId(), "parent_campaign_id" => $this->get("parent_campaign_id"), "parent_sub_id" => $this->getParentSubId(), "is_sale" => (int) $this->isSale(), "is_lead" => (int) $this->isLead(), "is_rejected" => (int) $this->isRejected(), "lead_revenue" => $this->get("lead_revenue"), "sale_revenue" => $this->get("sale_revenue"), "rejected_revenue" => $this->get("rejected_revenue")];
        for ($i = 1; $i <= Model\Click::getSubIdCount(); $i++) {
            $data["sub_id_" . $i] = $this->getSubIdN($i);
        }
        for ($i = 1; $i <= Model\Click::EXTRA_PARAM_COUNT; $i++) {
        ...
        $tokenParams = Repository\ParameterRepository::getAvailableParameters();
        $tokenParams[] = "ts_id";
    ...
    public function getFilteredDestination()
    {
        ...
        if (is_string($destination) && DESTINATION_LIMIT < mb_strlen($destination)) {
            $destination = mb_substr($destination, 0, DESTINATION_LIMIT, "utf-8") . "...";
        }
        return $destination = mb_substr($destination, 0, DESTINATION_LIMIT, "utf-8");
    }
}
