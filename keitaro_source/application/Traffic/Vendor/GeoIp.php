...
@define("GEOIP_SHM_KEY", 1329681409);
@define("US_OFFSET", 1);
...
function geoip_load_shared_mem($file)
{
    ...
    $shmid = shmop_open(GEOIP_SHM_KEY, "c", 420, $size);
...
function _setup_segments($gi)
{
    ...
    if ($gi->flags & GEOIP_SHARED_MEMORY) {
        ...
        $i = 0;
        while ($i < STRUCTURE_INFO_MAX_SIZE) {
            $delim = @shmop_read($gi->shmid, $offset, 3);
            $offset += 3;
            if ($delim == chr(255) . chr(255) . chr(255)) {
                $gi->databaseType = ord(@shmop_read($gi->shmid, $offset, 1));
                if (106 <= $gi->databaseType) {
                    $gi->databaseType -= 105;
                }
                $offset++;
        ...
        if ($gi->databaseType == GEOIP_COUNTRY_EDITION || $gi->databaseType == GEOIP_COUNTRY_EDITION_V6 || $gi->databaseType == GEOIP_PROXY_EDITION || $gi->databaseType == GEOIP_NETSPEED_EDITION) {
...
function geoip_open($filename, $flags)
{
    ...
    if ($gi->flags & GEOIP_SHARED_MEMORY) {
        ...
        ($gi->filehandle = fopen($filename, "rb")) || trigger_error("GeoIP API: Can not open " . $filename . "\n", 256);
...
function _geoip_seek_country_v6($gi, $ipnum)
{
    ...
    while (0 <= $depth) {
        if ($gi->flags & GEOIP_MEMORY_CACHE) {
            $buf = _safe_substr($gi->memory_buffer, 2 * $gi->record_length * $offset, 2 * $gi->record_length);
        } else {
            if ($gi->flags & GEOIP_SHARED_MEMORY) {
                $buf = @shmop_read($gi->shmid, 2 * $gi->record_length * $offset, 2 * $gi->record_length);
            } else {
                fseek($gi->filehandle, 2 * $gi->record_length * $offset, SEEK_SET) == 0 || trigger_error("GeoIP API: fseek failed", 256);
        ...
        $idx = $bnum >> 3;
        $b_mask = 1 << ($bnum & 7 ^ 7);
        if (0 < ($v6vec[$idx] & $b_mask)) {
            if ($gi->databaseSegments <= $x[1]) {
                return $x[1];
            }
            $offset = $x[1];
        } else {
            if ($gi->databaseSegments <= $x[0]) {
                return $x[0];
            }
            $offset = $x[0];
        }
    ...
    trigger_error("GeoIP API: Error traversing database - perhaps it is corrupt?", 256);
    return false;
}
