...
class Ip2LocationDownloadManager extends \Component\GeoDb\DownloadManager\DownloadManager implements \Component\GeoDb\DownloadManager\DownloadManagerInterface
{
    const DB_CODE = "db_code";
    const TOKEN = "token";
    const DOWNLOAD_TIMEOUT = 300;
    const API_URL = "https://www.ip2location.com/download?token=%token%&productcode=%dbname%";
    const INFO_URL = "https://www.ip2location.com/download-info?token=%token%&productcode=%dbname%";
    public function status()
    {
        ...
        if ($data[0] == "INVALID") {
            return [\Component\GeoDb\GeoDbStatus::ERROR, \Core\Locale\LocaleService::instance()->t("geo_dbs.errors.incorrect_token")];
        }
        if ($data[0] == "NOPERMISSION") {
            return [\Component\GeoDb\GeoDbStatus::ERROR, \Core\Locale\LocaleService::instance()->t("geo_dbs.errors.no_permission")];
        }
        if ($data[0] != "OK") {
    ...
    private function _getDbCode()
    {
        return $this->option(DB_CODE);
    }
    private function _getRemoteFileInfoResponse()
    {
        \Traffic\Logging\Service\LoggerService::instance()->debug("Request info: " . $this->getInfoUrl());
    ...
    private function _getRemoteVersion()
    {
        ...
        throw new \Component\GeoDb\Error\DbError("Status " . $body[0]);
    }
    ...
    {
        $url = str_replace("%token%", $this->_getToken(), $url);
        $url = str_replace("%dbname%", $this->_getDbCode(), $url);
        return $url;
    }
}
